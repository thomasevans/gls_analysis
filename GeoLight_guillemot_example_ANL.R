...
# Important here to choose a starting angle which is close to expected value. The function runs as a itterative process, finding a local 'mimia' in latitudinal variance. If you start too far from the 'true' value then you may get a wrong elevation value. I would suggest trying with a few different starting values in range -2:-7, where -2 represents high levels of shading, and -6 near 'perfect' conditions with no shading.
...
# Here we replace in missing values with some value (-4 here)
